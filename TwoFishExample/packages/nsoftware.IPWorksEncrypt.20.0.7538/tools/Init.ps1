...
[System.Reflection.Assembly]::LoadFile("$($installPath)\lib\net20\nsoftware.IPWorksEncrypt.dll") | Out-Null

If ($framework.Identifier.Equals(".NETFramework")) {
  [nsoftware.IPWorksEncrypt.IPWorksEncrypt]::CheckLicense("nuget")
}Else{
  [nsoftware.IPWorksEncrypt.IPWorksEncrypt]::CheckLicense("nugetcore")
}
<#                      Identifer                       Version
